                ...
                {vendors.map((vendor) => (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center mt-3 text-[10px] font-mono text-gray-500">
                      ...
                      <span className="font-bold text-navy-900 dark:text-gold-400">₹{vendor.revenue.toLocaleString()} sales</span>
                    </div>
                  </button>
                ))}
              </div>
                ...
                <div className="p-6 rounded-3xl border border-gray-100 dark:border-navy-900 bg-white dark:bg-navy-950 space-y-6">
                    ...
                    <div className="flex gap-2">
                      {!selectedVendorForDashboard.approved ? (
                        <button
                          ...
                          className="px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white font-display font-bold text-[10px] uppercase tracking-wider rounded-lg transition shadow-xs flex items-center gap-1 cursor-pointer"
                        >
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div className="p-3.5 rounded-xl bg-gray-50 dark:bg-navy-900 border text-left">
                      ...
                      <span className="text-base font-bold font-mono text-slate-800 dark:text-white">₹{selectedVendorForDashboard.revenue}</span>
                    </div>

                    <div className="p-3.5 rounded-xl bg-gray-50 dark:bg-navy-900 border text-left">
                      <span className="text-[9px] font-mono text-gray-450 dark:text-gray-505 uppercase block">Commission Due</span>
                      <span className="text-base font-bold font-mono text-gold-500">₹{Math.round(selectedVendorForDashboard.revenue * (selectedVendorForDashboard.commissionRate / 100))}</span>
                    </div>
                        ...
                        .map(p => (
                          <div key={p.id} className="py-3 flex justify-between items-center text-xs">
                            ...
                            <div className="text-right">
                              ...
                              <span className="text-[10px] font-mono text-gray-400 block mt-0.5">₹{p.price}</span>
                            </div>
                          </div>
                        ))}
                    </div>
            ...
            <div className="space-y-3">
              {bulkInquiries.length === 0 ? (
                ...
                bulkInquiries.map((inq) => (
                  <div
                    key={inq.id}
                    className="p-5 bg-white dark:bg-navy-950 rounded-2xl border border-gray-100 dark:border-navy-900 shadow-xs flex flex-col md:flex-row justify-between items-start md:items-center gap-4"
                  >
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
