...
export default function AccountPanel({
  ...
  // Authentication configuration: standard, phone OTP, or Google SSO
  ...
  const [subTab, setSubTab] = useState<'profile' | 'orders' | 'tracking' | 'wishlist' | 'coupons' | 'returns' | 'emails'>('profile');
  ...
  useEffect(() => {
    ...
    const fetchEmails = async () => {
      try {
        const res = await fetch(`/api/emails?recipient=${encodeURIComponent(currentUser.email)}`);
        if (res.ok) {
  ...
  const checkRateLimit = (): boolean => {
    ...
    setSubmitAttempts(prev => {
      ...
      if (next >= 4) {
        ...
        setRateLimitMessage("🔴 Security lockout active. Too many attempts. Try again in 25 seconds.");
        return 0;
      }
      setRateLimitMessage('');
      return next;
    ...
    if (authMethod === 'password') {
      ...
      if (!otpSent) {
        if (!phoneNumber.trim() || phoneNumber.length < 10) {
          setOtpError('Please input a valid 10-digit mobile number.');
          return;
        }
        setOtpSent(true);
        setOtpError('');
        // Provide user feedback showing the simulated OTP SMS
        alert(`🔒 SECURE GATEWAY TRANSMISSION\n\nSMS verification One-Time Code: 2026\nsent securely to +91 ${phoneNumber}`);
      } else {
        if (otpCode.trim() === '2026') {
          onLogin(`${phoneNumber}@meris-phone.com`, name.trim() || `Guest +91 ${phoneNumber.substring(6)}`);
          setOtpSent(false);
          setOtpCode('');
        } else {
          setOtpError('Invalid secure verification OTP. Code is 2026');
        }
      }
    }
  };
        ...
        <div className="bg-white dark:bg-navy-900 rounded-3xl p-6 sm:p-8 border border-gray-100 dark:border-navy-800 shadow-2xl space-y-6">
          ...
          {!isSignUp && (
            <div className="flex gap-2 p-1 bg-gray-50 dark:bg-navy-950 rounded-xl border border-gray-200/50">
              <button
                ...
                className={`flex-1 py-1.5 rounded-lg text-[11px] font-semibold transition cursor-pointer text-center ${authMethod === 'password' ? 'bg-white dark:bg-navy-800 text-gold-500 shadow-sm border border-gray-150' : 'text-gray-400 hover:text-navy-900 font-normal'}`}
              >
                Password Login
              </button>
              <button
                ...
                className={`flex-1 py-1.5 rounded-lg text-[11px] font-semibold transition cursor-pointer text-center ${authMethod === 'otp' ? 'bg-white dark:bg-navy-800 text-gold-500 shadow-sm border border-gray-150' : 'text-gray-400 hover:text-navy-900 font-normal'}`}
              >
                SMS OTP Safe
              </button>
            </div>
          ...
          {authMethod === 'password' || isSignUp ? (
            ...
            <form onSubmit={handleLoginSubmit} className="space-y-4">
              ...
              {!otpSent ? (
                <div>
                  <label className="block text-[10px] font-mono tracking-wider uppercase text-gray-400 mb-1">Mobile Number (+91)</label>
                  <div className="relative">
                  ...
                  <p className="text-[10px] text-gray-400 mt-1 font-mono">Verify instantly with secure passcode sandbox</p>
                </div>
              ) : (
                <div className="space-y-3">
                  <div className="p-3 bg-amber-50 dark:bg-navy-950 border border-amber-200 dark:border-navy-800 rounded-xl text-[11.5px] text-amber-700 dark:text-gold-300 font-sans leading-relaxed text-left">
                    💬 Secure passcode dispatched to <strong className="font-mono text-xs">+91 {phoneNumber}</strong>.
                    Type <strong>2026</strong> below to prove ownership.
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-9 pr-3 py-2 text-xs font-mono font-bold tracking-widest text-center border border-gray-200 rounded-xl focus:ring-1 focus:ring-gold-400 focus:outline-none dark:bg-navy-950 dark:border-navy-800 dark:text-white"
                      />
                    </div>
                  </div>
                </div>
              ...
              >
                <LogIn className="w-4 h-4 text-navy-950" />
                <span>{otpSent ? 'Confirm Secure Access' : 'Transmit SMS Passcode'}</span>
              </button>
              ...
              {otpSent && (
                ...
                >
                  Change Mobile Number coordinates
                </button>
              )}
            </form>
          )}
          ...
          <AnimatePresence mode="wait">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4 text-xs">
                    ...
                    <div className="p-4 rounded-xl bg-gray-50 border">
                      ...
                      <span className="font-bold text-emerald-600 tracking-wide mt-1 block flex items-center gap-1">🛡 ACTIVE MEMBER</span>
                    </div>
                  </div>
                </div>
                      ...
                      <div key={ord.id} className="p-4 rounded-2xl border border-gray-100 bg-white shadow-sm space-y-4">
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-gray-100 pb-3 gap-2 text-xs">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-mono font-bold text-navy-950 text-xs">Total: ₹{ord.total}</span>
                          </div>
                        </div>
                        ...
                        <div className="space-y-2 text-xs">
                          {ord.items.map((it) => (
                            <div key={it.product.id} className="flex justify-between items-center bg-gray-50/50 p-2 rounded-lg">
                              ...
                              <span className="text-gray-400 font-mono">₹{(it.product.discountPrice || it.product.price) * it.quantity}</span>
                            </div>
                          ))}
                        </div>
                      </div>
            ...
            {subTab === 'tracking' && (
              ...
              >
                        ...
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-xl border text-[10px] font-mono font-semibold transition cursor-pointer flex items-center gap-1.5 ${
                            searchedOrder?.id === o.id
                              ? 'bg-gold-50 border-gold-400 text-gold-700 font-bold shadow-xs'
                ...
                {searchedOrder ? (
                  <div className="p-6 rounded-2xl border border-gray-100 bg-white shadow-xs space-y-6">
                    {/* Header meta */}
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-gray-50/50 p-4 rounded-xl gap-4 border border-dashed">
                      ...
                      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full sm:w-auto">
                        <div className="text-left sm:text-right font-sans shrink-0">
                          ...
                          <h4 className="text-xs font-bold text-navy-950">₹{searchedOrder.total}</h4>
                        ...
                        <button
                          ...
                          className="w-full sm:w-auto px-4 py-2.5 bg-white hover:bg-gray-50 border border-gray-200 text-slate-800 hover:text-navy-950 font-display font-bold text-[10px] uppercase tracking-wider rounded-xl transition cursor-pointer active:scale-95 shadow-xs flex items-center justify-center gap-1.5 shrink-0"
                        ...
                        <div className="space-y-6">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            ...
                            <div className="p-3 bg-white border rounded-xl text-left font-sans">
                              ...
                              <span className="text-xs font-bold text-emerald-600 block mt-0.5 flex items-center gap-1">✓ Verified</span>
                            </div>
                            <div className="p-3 bg-white border rounded-xl text-left font-sans">
                    ...
                    <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs font-sans">
                      <div className="space-y-1">
                        <h5 className="font-display font-bold text-slate-500 uppercase tracking-widest text-[9px] flex items-center gap-1.5">
                          ...
                          🧑‍💻 Developer Override - Simulate Partner Handoff
                        </h5>
                      ...
                      <div className="space-y-1.5 font-sans">
                        {searchedOrder.items.map((it) => (
                          <div key={it.product.id} className="flex justify-between items-center text-xs bg-gray-50 p-2.5 rounded-lg">
                            ...
                            <span className="font-mono text-gray-500">₹{(it.product.discountPrice || it.product.price) * it.quantity}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                {wishlistProducts.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {wishlistProducts.map((prod) => (
                      <div key={prod.id} className="p-3.5 rounded-2xl border border-gray-100 bg-white shadow-xs flex items-center gap-3 justify-between">
                        <div className="flex items-center gap-3 cursor-pointer" onClick={() => onSelectProduct(prod.id)}>
                          ...
                          <div className="text-left font-sans space-y-0.5">
                            ...
                            <span className="text-xs font-bold text-navy-900 block mt-0.5">₹{prod.discountPrice || prod.price}</span>
                          </div>
                        </div>
                  ...
                  {coupons.map((c) => (
                    <div key={c.code} className="p-4 rounded-2xl border-2 border-dashed border-gold-400/30 bg-gradient-to-br from-gold-50/20 to-transparent flex flex-col justify-between text-left space-y-3 relative overflow-hidden">
                      ...
                      <div className="space-y-1 relative z-10 text-xs">
                        ...
                        <p className="text-[10px] text-gray-400">Min order cart value: ₹{c.minimumCartValue}</p>
                      </div>
                  ...
                  <form onSubmit={handleTriggerRefundRequest} className="space-y-4 max-w-md">
                    ...
                    <div>
                      ...
                      >
                        ...
                        {orders.map(o => (
                          <option key={o.id} value={o.id}>{o.orderNumber} (₹{o.total})</option>
                        ))}
                      </select>
                    </div>
              ...
              >
                ...
                {emails.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                    ...
                    <div className="lg:col-span-5 space-y-2 max-h-[500px] overflow-y-auto pr-1">
                      {emails.map((emailItem) => (
                        <button
                          ...
                          className={`w-full text-left p-4.5 rounded-2xl border transition flex flex-col gap-2 relative overflow-hidden group cursor-pointer ${
                    ...
                    <div className="lg:col-span-7 bg-white border border-gray-100 rounded-3xl p-4.5 shadow-sm space-y-4">
                      {selectedEmail ? (
                        <div className="space-y-3">
              ...
              <div className="p-6 md:p-8 space-y-6 max-h-[70vh] overflow-y-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-sans text-navy-950">
                  <div className="p-4 bg-gray-50 border rounded-2xl">
                    ...
                    <p className="text-[10px] font-mono text-gray-400 mt-2">☎ {selectedDetailsOrder.customerInfo.phone}</p>
                  </div>
                ...
                <div className="bg-gray-50 border rounded-2xl p-4 md:p-5 space-y-2.5 text-xs text-navy-950">
                  ...
                  <div className="flex justify-between items-center text-gray-600">
                    <span>Subtotal</span>
                    <span className="font-mono">₹{selectedDetailsOrder.subtotal}</span>
                  </div>
                  ...
                  {selectedDetailsOrder.discount > 0 && (
                    <div className="flex justify-between items-center text-emerald-600">
                      ...
                      <span className="font-mono font-semibold">-₹{selectedDetailsOrder.discount}</span>
                    </div>
                  )}

                  <div className="flex justify-between items-center text-gray-600">
                    <span>Shipping Handling Cost</span>
                    <span className="font-mono">₹{selectedDetailsOrder.shippingCost}</span>
                  </div>

                  <div className="flex justify-between items-center text-gray-600">
                    <span>GST (Goods & Services Tax)</span>
                    <span className="font-mono">₹{selectedDetailsOrder.tax}</span>
                  </div>
                  ...
                  <div className="flex justify-between items-center border-t pt-3 font-display font-bold text-sm text-navy-900">
                    ...
                    <span className="font-mono text-gold-600">₹{selectedDetailsOrder.total}</span>
                  </div>
                </div>
              </div>
